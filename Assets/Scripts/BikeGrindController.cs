...
namespace SBPScripts
{
    ...
    public class BikeGrindController : MonoBehaviour
    {
        ...
        private SphereCollider rWheelCollider;
        ...
        private void InitializeReferences()
        {
            ...
            if (rPhysicsWheel != null)
            {
                ...
                rWheelCollider = rPhysicsWheel.GetComponent<SphereCollider>();
                
                if (rWheelCollider == null)
                {
                    Debug.LogError("BikeGrindController: RPhysicsWheel needs a SphereCollider for rail detection!");
                }
            }
            else
            {
        ...
        {
            // Check if back wheel hit a rail
            if (collision.gameObject.CompareTag("Rail") && !isGrinding)
            {
                // Verify collision was with back wheel
                if (collision.collider == rWheelCollider)
                {
                    if (showDebug) Debug.Log($"<color=green>Rail collision detected on back wheel!</color>");
                    StartGrind(collision.gameObject);
                }
            }
        }
        ...
        private void ExitGrind(bool jumped)
        {
            ...
            // Resume physics first
            ResumePhysics();
            
            // Apply exit velocity
            if (jumped)
            {
                // Jump off rail - add upward force and maintain forward momentum
                Vector3 exitVelocity = transform.forward * velocityBeforeGrind.magnitude * exitForceMultiplier;
                exitVelocity.y += 5f; // Upward jump force
                mainRigidbody.linearVelocity = exitVelocity;
            }
            else
            {
                // Natural exit - maintain velocity in current direction
                Vector3 exitVelocity = transform.forward * grindSpeed * exitForceMultiplier;
                mainRigidbody.linearVelocity = exitVelocity;
            }
